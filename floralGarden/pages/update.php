...
<main class="personal_main">
    <section class="container my-5">
        ...
        <?php
            ...
            $id = $_GET['id'];
            $cards = $mysql->query("SELECT * FROM `goods` WHERE `id` = '$id'");
            $card = mysqli_fetch_assoc($cards);
 
        ?>
        <form class="row g-3 needs-validation" method="POST" action="/php/redact.php" novalidate>
            <div class="col-md-12">
            ...
            <div class="col-md-12">
                ...
                <input type="text" value="<?=$card['name']?>" name="name" class="form-control" id="validationCustom01"  required>
            </div>
            <div class="col-md-12">
                <label for="validationCustom01" class="form-label">Описание</label>
                <input type="text" value="<?=$card['description']?>" name="description" class="form-control" id="validationCustom01"  required>
            </div>
            <div class="col-md-12">
                <label for="validationCustom01" class="form-label">Изображение</label>
                <input type="text" value="<?=$card['img']?>" name="img" class="form-control" id="validationCustom01"  required>
            </div>
            <div class="col-md-12">
                <label for="validationCustom01" class="form-label">Цена</label>
                <input type="number" value="<?=$card['price']?>" name="price" class="form-control" id="validationCustom01"  required>
            </div>
            <div class="col-md-12">
                <label for="validationCustom01" class="form-label">Категория</label>
                <input type="number" value="<?=$card['category']?>" name="category" class="form-control" id="validationCustom01"  required>
            </div>
            <div class="col-12">
